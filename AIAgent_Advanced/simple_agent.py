...
# 아래의 주석은 agent를 실행하는 것을 사용자가 직접 하나하나 단계를 쪼개어 실행해본 결과
...
# agent에 조금 더 복잡한 문제를 내게 된다면, 제대로 응답을 하지 못하는데
...
# 사용자가 하나씩 끊어서 하지 말고 Agent가 자체적으로 내부에 로직을 돌면서 실행하도록 하면 아래와 같이 코드로 작성할 수 있고
...
# 사용자의 입력을 사이사이에 추가하여 agent가 다음 행동을 진행할 수 있도록 변경된 agent
...
# 위의 경우는 이제 langchain에서 제공하는 agentexecutor를 이용하면 코드 한 줄로 간단히 처리할 수 있다!!!
# agent와 도구목록, 반복되는 횟수의 한계를 제공하여, 생성한 뒤에 executor에 사용자의 질문을 집어넣고 실행하면 간단히 처리할 수 있음!
